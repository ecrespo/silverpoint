...
type Prop<K extends keyof GaugeArcProps> = GaugeArcProps[K];

/**
 * `<sp-gauge-arc>` (REQ-080): standalone, signal inputs, OnPush (REQ-101). Inputs are named
 * as in every other adapter (API Spec §4).
 */
@Component({
  selector: 'sp-gauge-arc',
...
export class SpGaugeArc extends SpChart<GaugeArcProps> {
