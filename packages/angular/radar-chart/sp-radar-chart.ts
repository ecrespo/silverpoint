import { ChangeDetectionStrategy, Component, computed, input } from '@angular/core';
import { SpChart, SpChartFrame, SpChartOverlay } from '@silverpoint/angular';
import { radarChart, type RadarChartProps } from '@silverpoint/core';

type Prop<K extends keyof RadarChartProps> = RadarChartProps[K];

/**
 * `<sp-radar-chart>` (REQ-076): standalone, signal inputs, OnPush (REQ-101). Inputs are named
 * as in every other adapter (API Spec §4).
 */
@Component({
  selector: 'sp-radar-chart',
  imports: [SpChartFrame, SpChartOverlay],
  changeDetection: ChangeDetectionStrategy.OnPush,
  template: `
    <sp-chart-frame [rendered]="rendered()" [rootClass]="rootClass()" [interactive]="true" (rootEvent)="onRootEvent($event)">
      <sp-chart-overlay [rendered]="rendered()" [active]="active()" [tooltip]="tooltip()?.template" />
    </sp-chart-frame>
  `,
})
export class SpRadarChart extends SpChart<RadarChartProps> {
  readonly subjectKey = input<Prop<'subjectKey'>>();
  readonly valueKey = input<Prop<'valueKey'>>();
  readonly domain = input<Prop<'domain'>>();

  protected readonly ownProps = computed(() => ({
    subjectKey: this.subjectKey(),
    valueKey: this.valueKey(),
    domain: this.domain(),
  }));

  constructor() {
    super(radarChart);
  }
}
