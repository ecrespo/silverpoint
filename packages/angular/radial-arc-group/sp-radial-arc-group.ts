...
type Prop<K extends keyof RadialArcGroupProps> = RadialArcGroupProps[K];

/**
 * `<sp-radial-arc-group>` (REQ-078): standalone, signal inputs, OnPush (REQ-101). Inputs are named
 * as in every other adapter (API Spec §4).
 */
@Component({
  selector: 'sp-radial-arc-group',
...
export class SpRadialArcGroup extends SpChart<RadialArcGroupProps> {
