import { ChangeDetectionStrategy, Component, computed, input } from '@angular/core';
import { SpChart, SpChartFrame, SpChartOverlay } from '@silverpoint/angular';
import { sparklineRows, type SparklineRowsProps } from '@silverpoint/core';

type Prop<K extends keyof SparklineRowsProps> = SparklineRowsProps[K];

/**
 * `<sp-sparkline-rows>` (REQ-062): standalone, signal inputs, OnPush (REQ-101). Inputs are named
 * as in every other adapter (API Spec §4).
 */
@Component({
  selector: 'sp-sparkline-rows',
  imports: [SpChartFrame, SpChartOverlay],
  changeDetection: ChangeDetectionStrategy.OnPush,
  template: `
    <sp-chart-frame [rendered]="rendered()" [rootClass]="rootClass()" [interactive]="true" (rootEvent)="onRootEvent($event)">
      <sp-chart-overlay [rendered]="rendered()" [active]="active()" [tooltip]="tooltip()?.template" />
    </sp-chart-frame>
  `,
})
export class SpSparklineRows extends SpChart<SparklineRowsProps> {
  readonly rows = input<Prop<'rows'>>();
  readonly nameKey = input<Prop<'nameKey'>>();
  readonly readoutKey = input<Prop<'readoutKey'>>();
  readonly seriesKey = input<Prop<'seriesKey'>>();
  readonly pointKey = input<Prop<'pointKey'>>();

  protected readonly ownProps = computed(() => ({
    rows: this.rows(),
    nameKey: this.nameKey(),
    readoutKey: this.readoutKey(),
    seriesKey: this.seriesKey(),
    pointKey: this.pointKey(),
  }));

  constructor() {
    super(sparklineRows);
  }
}
