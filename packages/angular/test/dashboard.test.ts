...
describe('sp-dashboard under @angular/platform-server (T-113)', () => {
  ...
  test('REQ-205 · a dashboard whose children match its layout warns nothing, static or generated by @for', async () => {
    const seen = capture();
    await ssr(OPS);
    expect(seen).toEqual([]);
    await ssr(`
      <sp-dashboard id="ops" title="Operations" [layout]="layout">
        @for (id of ['traffic', 'revenue']; track id) {
          <sp-dashboard-cell [cell]="id"><sp-kpi-card [title]="id" /></sp-dashboard-cell>
        }
      </sp-dashboard>`);
    expect(seen).toEqual([]);
  });
