...
| T-085 | done | b3f09ae | fixture test RED (catalog pinned at 33) → CARDS + renderers for the 12 → canonicals for 264 fixtures (the 21 earlier charts' unchanged); tools 288/288; gates: string gate over 33 charts × 8 × 3 adapters, subpaths and tree-shaking green |
| Demo leak (Phase 1 defect) | done | b3f09ae | checking the new wind demo in other bundles showed **every** chart's demo in **every** one-chart bundle since Phase 1: the `@__PURE__` on `Object.freeze(…)` dropped the call but kept its argument `[…].map(…)`, which the bundler cannot prove pure — the gate only looked for recipe names. RED: a demo check in the tree-shaking gate (132/132 red) → `/* @__PURE__ */` on the inner call of all 31 demos → green; the check was then made generic (each chart's demo words that no other demo uses, split from its table and labels, plus field names for numeric demos) and mutation-checked (the donut's annotation removed → 128 bundles red). Heaviest one-chart bundle 35.1 → 31.2 kB |
| T-089 (budgets) | done | b3f09ae | the per-chart budget coverage test RED on the 12 (48 paths) → 48 `.size-limit.json` entries at 45 kB; the catalog-wide core alarm 40 → 45 kB (measured 42.1 kB with the catalog complete) |
| Gate race | done | b3f09ae | a full run failed `check-deps` once: the gates' resolution test rewrites the real `packages/grounds/package.json` to prove the stylesheet check while the tools project reads it in parallel (a Phase 0 race). The gates project now runs after the unit projects (`sequence.groupOrder: 1`); full run 2477/2477 |
| T-086 | done | (this commit) | e2e apps RED (48 = 12 × 4 apps) → the 12 in every app's `CHARTS` and the canonical page's `RECIPES` → 150/150; 96 goldens via `--update-snapshots=missing` in the pinned image (run alone), looked at (12 cards, no overprint), compare-only 1064/1064 |
| T-087 | done | (this commit) | the a11y spec iterates every fixture and counts the gallery from `CATALOG`: 300/300 — axe-core clean on the 33-chart gallery, tables and Tab + arrows announcing for every chart |
| T-088 | done | (this commit) | `changes/phase-3-req-124-review.md` (a channel table per chart; three carry the stacked-bar limit on paper); 10 precision tests + the no-hatch test over all 33, written after the recipes and mutation-checked in two batches (10 mutations, each caught) |
| T-089 | done | (this commit) | the 12 in the grounds equivalence suite (134/134, REQ-006); `POLAR_AT_CEILING` benchmark datasets (60 sectors, 12 chord categories, 100 observations / markers) RED → green, 29/29; the nightly benchmark: all 27 within 2 ms (slowest polar 0.46 ms); budgets and tree-shaking in b3f09ae; full run 2548/2548, lint and typecheck clean |
