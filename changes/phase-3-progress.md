...
| T-083 | done | (this commit) | `OrbitChart` per `changes/delta-009-orbit-props.md` (PROPOSED: how `orbits`, `markerKey`, `periodKey` read Data Model §2.10). Contract case (13 tests) + 7 specific tests RED on the missing recipe → green; the case first landed in the cartesian list (an insertion anchor matched twice) and drew the 500-point contract — moved; preview looked at; core 747/747 |
