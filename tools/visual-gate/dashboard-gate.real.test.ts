...
describe('the dashboard tree gate over the published builds', () => {
  test('REQ-210 · REQ-212 · every dashboard fixture is identical across 3 adapters × 2 modes × 4 substrates', async () => {
    const fixtures = loadDashboardFixtures();
    expect(fixtures).toHaveLength(DASHBOARDS.length * 8);
    ...
    expect(results).toHaveLength(DASHBOARDS.length * 8 * 3);
  }, 600_000);
});
