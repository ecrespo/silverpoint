...
> Source specs: [PRD](prd.md) v1.5 · [API Spec](api-spec.md) v1.3 ·
> [Technical Design](technical-design.md) v1.2 · [Data Model](data-model.md) v1.1 ·
> [Implementation Plan](implementation-plan.md) v1.1
> Plan phase covered: **Phase 0** · Generated: 2026-09-13 · Revised: 2026-09-13
...
- **Agent execution: the first batch is T-001 to T-004, then review, adjust, and only then
  scale.** Do not launch all 27 unsupervised.
...
- **What**: pnpm workspaces plus Nx; the five allowlist packages with their `package.json`, `strict` `tsconfig` and `exports`; React and Angular declared as `peerDependencies`; empty `examples/` and `tools/`.
- **REQ**: REQ-160, REQ-161, REQ-163
- **Files**: `pnpm-workspace.yaml`, `nx.json`, `packages/*/package.json`
- **Depends on**: —
- **Done**: `pnpm -r build` green with empty packages; no adapter declares React or Angular as a dependency.
...
**[ ] T-017 · Readout wired in both adapters** ⟵ *closes Analyze finding A-03*
- **What**: connect the T-009 engine to pointer and keyboard events in both adapters; emit `onActiveChange` / `activeChange`; default readout plus the consumer-supplied renderer slot.
- **REQ**: REQ-141, REQ-142, REQ-143
- **Files**: `packages/react/src/interaction/`, `packages/angular/src/interaction/`
- **Depends on**: T-009, T-015, T-016
...
**[ ] T-019 · Cross-adapter string gate** ⟵ *the risk-retiring task*
- **What**: SSR render of both adapters under Node, compared through the normaliser.
- **REQ**: REQ-103, REQ-180
- **Files**: `tools/visual-gate/string-gate.ts`
- **Depends on**: T-015, T-016, T-018
- **Done**: the line chart is identical across the 2 modes × 4 substrates. **If it is not, the phase stops and the plan's stop condition applies.**

**[ ] T-020 · Fixture matrix**
- **What**: the schema from Data Model §5 and the line chart's fixtures.
- **REQ**: REQ-182
- **Files**: `fixtures/`
- **Depends on**: T-008
- **Done**: 8 line-chart fixtures load and validate against the schema.
...
**[ ] T-022 · Example apps with a shared harness**
- **What**: `vite-react`, `nextjs` and `angular`, all three on **the same stylesheet** and the same fixed-size container.
- **REQ**: REQ-103
- **Files**: `examples/*`
- **Depends on**: T-015, T-016
- **Done**: all three boot and show the line chart; Next.js hydrates with no mismatch.

### Phase close-out

**[ ] T-023 · `ink` / `precision` equivalence**
...
**[ ] T-024 · Line chart accessibility**
...
- **Done**: `axe-core` reports no A or AA issues across all three apps; forcing `prefers-contrast: more` switches to `precision`; the chart is legible with hatching disabled.

**[ ] T-025 · Path-weight measurement** ⟵ *the missing number*
...
**[ ] T-026 · Bundle budgets** `[P]`
- **What**: `size-limit` across the five packages.
- **REQ**: REQ-164
- **Depends on**: T-015, T-016
- **Done**: `core` + `react` with the line chart under 45 KB; exceeding it breaks CI.

**[ ] T-027 · Automated traceability**
...
| REQ-095 | T-008, T-015, T-016 |
| REQ-100, REQ-104, REQ-107 | T-015 |
| REQ-101, REQ-102, REQ-105 | T-016 |
| REQ-103 | T-019, T-022 |
| REQ-120, REQ-121, REQ-122, REQ-123, REQ-124, REQ-125 | T-024 |
| REQ-126, REQ-127 | T-011 |
| REQ-140, REQ-144 | T-009 |
| REQ-141, REQ-142, REQ-143 | T-017 |
| REQ-160, REQ-161, REQ-163 | T-001 |
| REQ-164 | T-026 |
| REQ-180 | T-018, T-019 |
| REQ-181 | T-021 |
| REQ-182 | T-020 |
| REQ-183, REQ-184 | T-027 |
